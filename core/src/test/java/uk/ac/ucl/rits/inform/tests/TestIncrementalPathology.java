...
public class TestIncrementalPathology extends InterchangeMessageToDbTestCase {
    ...
    public void testResultsExist() {
        ...
        List<PatientFact> allTestResults001 = childFacts001.stream().filter(pf -> pf.isValid() && pf.isOfType(OldAttributeKeyMap.PATHOLOGY_TEST_RESULT)).collect(Collectors.toList());
        List<PatientFact> allTestResults002 = childFacts002.stream().filter(pf -> pf.isValid() && pf.isOfType(OldAttributeKeyMap.PATHOLOGY_TEST_RESULT)).collect(Collectors.toList());
    ...
    public void testResultsUpdate() {
        ...
        Map<String, List<PatientFact>> resultFactsByTestCode = resultFacts.stream()
                ...
                .collect(Collectors.groupingBy(
                        pf -> pf.getPropertyByAttribute(OldAttributeKeyMap.PATHOLOGY_TEST_CODE)
        ...
        List<PatientProperty> allProperties = monocyteResults.stream().map(mr -> mr.getPropertyByAttribute(OldAttributeKeyMap.PATHOLOGY_NUMERIC_VALUE).get(0)).collect(Collectors.toList());
