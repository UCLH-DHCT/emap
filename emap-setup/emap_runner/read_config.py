...
class ConfigFile:
    """
    A configuration file constructed from a existing .yaml
    """
    ...
    def __init__(self, filename: str):
        ...
        self._propagate_common_values()

    def _propagate_common_values(self) -> None:
        """
        ...
        for entry in self.config["informdb"]:
            ...
            for key in inform_common:
                self.config["informdb"][entry][key] = inform_common.get(key, " ")
    ...
    def repo_info(self) -> dict:
        ...
        for repo in self.config["repositories"]:
            if "repo_name" in self.config["repositories"][repo]:
    ...
    def get_data_for(
        self, dataname: str, element: str, outer_element: str = ""
    ) -> dict:
        """
        ...
        if dataname in self.config:
            ...
            outer_element in self.config
            and element in self.config[outer_element]
